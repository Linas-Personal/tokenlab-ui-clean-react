...
class TokenEconomy_Basic(TokenEconomy):
    ...
    Properties:
        ...
        transactions_value_in_fiat: the effective total transaction volume in the fiat currency.
    ...
    def __init__(
        ...
        fiat: str = "$",
        token: str = "token",
        price_function: PriceFunctionController = PriceFunction_EOE,
        price_function_parameters: Dict = {},
        supply_pools: List[SupplyController] = [],
        ...
        """
        super(TokenEconomy_Basic, self).__init__(
            ...
            token_initial_price=[initial_price],
            name=name,
        )
        ...
        if supply_is_added is None:
            raise Exception(
                "The parameter supply_is_added is not set! You need to make a decision. Setting supply_is_added=True ! This will work fine for most use cases, unless you want the supply to take certain values irrespective of the trading volume."
            )
    ...
    def add_agent_pool(self, agent_pool: AgentPool) -> bool:
        ...
        if agent_pool.currency != self.fiat:
            if np.logical_not(agent_pool.currency in self.tokens):
                raise Exception(
                    "The currency of this agent pool is neither the fiat, nor the token. Agent pool name :"
                    + agent_pool.name
                )
    ...
    def test_integrity(self) -> bool:
        ...
        for agent in self._agent_pools:
            ...
            if not res:
                raise Warning(
                    "Integrity of agent pool is not True. Name of pool: "
                    + str(agent.name)
                )
                return False
        ...
        if len(set(self._agent_pools)) < len(self._agent_pools):
            ...
            raise Warning(
                "duplicate agent pools detected! It is likely the simulation will not run successfuly!"
            )
    ...
    def execute(self) -> bool:
        ...
        Agent pools can also execute with negative transaction volumes, lowering the price.
        ...
        if not self.test_integrity():
            raise Exception(
                "Integrity of pools not correct. Please make sure all agent pools have the correct dependencies."
            )
        ...
        for agent in pools:
            ...
            if agent.currency == self.token:
                ...
                if tokens > 0:
                    ...
                    self.transactions_value_in_fiat += (
                        agent.get_transactions() * self.price
                    )
                ...
                if self.price > 0:
                    ...
                    warnings.warn(
                        "Warning! Price reached 0 at iteration : " + str(self.iteration)
                    )
                    return False
                ...
                if self.safeguard_current_supply_level:
                    if self.transactions_volume_in_tokens >= self.supply:
                        print(
                            "Warning! Demand surpassed supply at iteration : "
                            + str(self.iteration)
                        )
                ...
                raise Exception(
                    "Agent pool found that does not function in neither fiat nor the token! Please specify correct currency!"
                )
            ...
            # If the price is dynamic, then we calculate the price after every agent pool acts
        ...
        if self.burn_token:
            self.change_supply(
                self.token, -self.transactions_volume_in_tokens * self._burn_coefficient
            )
        ...
        self._effective_holding_time = (
            self.price * self.supply / (self.transactions_value_in_fiat + 0.000000001)
        )
        ...
        self._transactions_value_store_in_tokens.append(
            self.transactions_volume_in_tokens
        )
    ...
    def get_data(self) -> pd.DataFrame:
        ...
        package = {
            self.token + "_price": self._prices_store,
            "transactions_" + self.fiat: self._transactions_value_store_in_fiat,
            "num_transactions": self._num_transactions_store,
            "num_users": self._num_users_store,
            "iteration": np.arange(1, self.iteration + 1),
            "holding_time": self._holding_time_store,
            "effective_holding_time": self._effective_holding_time_store,
            "supply": self._supply_store,
            "transactions_" + self.token: self._transactions_value_store_in_tokens,
        }

        df = pd.DataFrame(package)

        temp_df = pd.DataFrame(
            self._treasury_store, columns=["treasury_name", "treasury_value"]
        )
        if self.treasuries is not None:
            for tres in self.treasuries:
                dummy_df = (
                    temp_df[temp_df["treasury_name"] == tres.name]
...
class TokenMetaSimulator:
    ...
    def get_report(
        self,
        functions: List = [np.mean, np.std, np.max, np.min, _quantile_10, _quantile_90],
        segment: Union[List, Tuple, int, str] = None,
    ) -> pd.DataFrame:
        """
        ...
        if segment == None:
            ...
            averages = self.data.loc[
                (self.data["iteration_time"] > segment[0])
                & (self.data["iteration_time"] < segment[1]),
                :,
            ]
            averages = averages.groupby("repetition_run").mean()
        elif segment == "last":
            averages = (
                self.data[self.data["repetition_run"] == self.repetitions]
                .groupby("repetition_run")
                .mean()
            )
    ...
    def get_timeseries(
        ...
        final = pd.DataFrame(
            {
                feature + "_mean": timeseries_mean.values,
                feature + "_median": timeseries_median.values,
                "sd": timeseries_std.values,
                "quant_10%": timeseries_quant_10,
...
class TokenEconomy_Dependent(TokenEconomy_Basic):
    ...
    def __init__(
        self,
        dependent_token_economy: TokenEconomy_Basic,
        holding_time: Union[float, HoldingTimeController],
        supply: Union[float, SupplyController],
        initial_price: float,
        fiat: str = "tokenA",
        ...
        super(TokenEconomy_Dependent, self).__init__(
            holding_time=holding_time,
            supply=supply,
...
class TokenEcosystem(TokenEconomy):
    ...
    def __init__(
        self,
        token_economies: List[TokenEconomy],
        ...
        for tokenec in self.token_economies:
            if tokenec.name == None:
                raise Exception(
                    "Master token economy and all other token economies need to have a name!"
                )
