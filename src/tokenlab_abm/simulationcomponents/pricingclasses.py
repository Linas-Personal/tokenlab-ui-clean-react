...
class HoldingTime_Stochastic(HoldingTimeController):
    ...
    def __init__(
        self,
        distribution: scipy.stats = scipy.stats.lognorm,
        holding_time_params: Union[Dict[str, float], List[Dict[str, float]]] = {
            "loc": 0,
            "s": 1,
        },
        minimum: float = 0.1,
...
class PriceFunction_EOE(PriceFunctionController):
    ...
    def execute(self, use_previous_supply: bool = True) -> float:
        ...
        # EoE: M * V = T * P
        # Example: In a simple one token-one fiat system, this can be represented as:
        # The supply of token * the velocity of token = the transaction volume in fiat * 1/the price of token
        # Use an empirical formula to get the velocity from holding time
        ...
        # Or simply use velocity = 1/holding time
        else:
            price_new = holding_time * transaction_volume_in_fiat / M

        # noise adjustment
        price_new = (
            self.smoothing_param * price_new
            + (1 - self.smoothing_param) * tokeneconomy.price
        )
...
class PriceFunction_IssuanceCurve(PriceFunctionController):
    ...
    def __init__(
        self,
        function: Callable,
        keep_internal_supply_state: bool = True,
...
class PriceFunction_LinearRegression(PriceFunctionController):
    ...
    def execute(self, use_previous_supply: bool = True) -> float:
        ...
        if T > 0:
            ...
            log_price_new = (
                0.88 * np.log(T) + 0.84 * np.log(1 / M) + 1.15 * np.log(1 / V)
            )
